...
| `success_response(data), 201` 写成双重嵌套 tuple | `success_response()` 内部已返回 `(jsonify(body), status_code)` tuple；若写成 `return success_response(data), 201`，外层 Python 再包一层 → `((Response, 200), 201)`，Flask 抛 `TypeError: view function did not return a valid response` 并返回 500 HTML，响应无结构极难诊断。**正确写法：`return success_response(data, code=201)`**，通过关键字参数传状态码 |
| 冒烟测试 FK 字段（`inspector_id` / `received_by` 等）硬编码 `id=1` | 种子数据中 admin user 的实际 id 取决于插入顺序，不一定是 1（本项目为 2）；直接用 `id=1` 触发 FK constraint violation → 500，报错信息不直接指向问题。**冒烟脚本应先 `GET /api/auth/me` 或查询用户列表取真实 ID，再用于需要用户 FK 的字段** |
...
| 2026-05-17 | Phase 3 Step 3-3-1：GoodsReceipt（actual_arrival_date=DATE，received_qty 保留，purchase_order_id 可选 FK）+ IqcReport（result 枚举 pass/fail/concession，inspector_id，inspection_date，file_path）+ EmergencyAuthRecord（iqc_report_id FK，authorized_by_pm/iqc，authorization_date NOT NULL，risk_description NOT NULL）三表 Migration bf7cdd51266e upgrade 通过；SHOW CREATE TABLE 确认 utf8mb4+FK；字段以 API spec §3 为准（TASKS 推导版与 spec 差异已逐项确认） | Claude |
| 2026-05-17 | Phase 3 Step 3-3-2：goods_receipt_service（JSON body，received_by=JWT identity，无文件）+ iqc_service（multipart，file_path 字段，remark，fail/concession TODO Phase 4 钩子）+ emergency_auth_service（JSON body，双签必填，risk_description 非空校验）+ blueprints/iqc.py（三 Blueprint 聚合，各 2 端点共 6 端点）；NotImplementedError→501 error handler 追加；修正 prompt 三处偏差（GoodsReceipt 无照片→JSON body、IqcReport file_path 字段名、去除 inspection_items/conclusion_note）；冒烟 A~J 全通过 | Claude |
| 2026-05-17 | Phase 3 Step 3-3-2 实战补入：§h 新增 2 条风险行（`success_response(data), 201` 双重嵌套 tuple 导致 Flask 500；冒烟 FK 字段硬编码 id=1 而种子数据 admin id=2 触发 FK constraint 500）；Windows 服务器管理实战教训写入 memory（端口占用检测、进程 PID kill、PowerShell 单块执行） | Claude |
